<template>
	<view class="details-container">
		...
		<view class="product-price" v-if="role == 'admin'">
          ...
          " " + formatNumberRgx(bagDetails.hkdPriceIndi)
        }}</text></text>
		...
		<view class="product-parameter" v-if="
        bagDetails.createTime ||
        bagDetails.cost ||
        bagDetails.pricePeer ||
        bagDetails.note
      ">
          ...
          bagDetails.createTime + " 【" + bagDetails.stockLoc + "】"
            ...
            "HKD " + formatNumberRgx(bagDetails.hkdCost)
          }}</text>
			...
			<view class="parameter-every" v-if="bagDetails.pricePeer && bagDetails.pricePeer != 0">
				...
				<view>
					<text style="color: #73d1c6">{{
            "HKD " + formatNumberRgx(bagDetails.hkdPricePeer)
          }}</text>
					<text v-if="role == 'admin'"
      ...
      ">
			...
			<view class="parameter-every" v-if="bagDetails.priceTran">
				...
				<view>
					<text style="color: #73d1c6">{{
            "HKD " + formatNumberRgx(bagDetails.hkdPriceTran)
          }}</text>
					<text v-if="role == 'admin'"
			...
			<view class="parameter-every" v-if="bagDetails.priceTran">
				...
				<view>
					<text style="color: #73d1c6">{{
            "HKD " +
            formatNumberRgx(bagDetails.hkdPriceTran - bagDetails.hkdCost)
          }}</text>
					<text v-if="role == 'admin'" style="color: #ff8b62; padding-left: 20rpx">{{
              "CNY " + formatNumberRgx(bagDetails.priceTran - bagDetails.cost)
            }}</text>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		data() {
			return {
				...
				share: {
					...
					path: "/minePackage/details?id=" + this.detailId,
					imageUrl: "",
					desc: "",
					content: "",
				},
		...
		onShareAppMessage(res) {
			this.share.path = "/minePackage/details?id=" + this.detailId;
			return {
		...
		methods: {
			...
			serviveClick() {
				uni.navigateTo({
					url: "../commonPackage/service/service?id=" + this.bagDetails.id,
				});
			},
		},
	};
