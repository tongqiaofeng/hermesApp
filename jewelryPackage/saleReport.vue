	...
	<view class="content">
			...
			<view class="inputs">
				...
				<view class="item-state">
					<view class="state-left">
						<view class="state" @click="stateClick(1)"
							:style="{color: state == 1 ? '#85dbd0' : '#000',borderBottom: state == 1 ? '2rpx solid #85dbd0' : '#f9f9f9',}">
							可结算</view>
						<view class="state" @click="stateClick(-1)"
							:style="{color: state == -1 ? '#85dbd0' : '#000',borderBottom: state == -1 ? '2rpx solid #85dbd0' : '#f9f9f9',}">
							不可结算</view>
		...
		<view v-else class="inventory-main" :style="{'padding-top': topHeight+'px'}">
			<view class="inventory-num">
				<view>总数量：{{ total }}</view>
				<view style="display: flex;justify-content: space-between;" v-if="state == 1 && currencyIdx == 0">
					<view>销售额：{{ getPrice(allSaleMoney, 'CNY') }}</view>
					<view>利润：{{ getPrice(allProfit, 'CNY') }}</view>
				</view>
				<view style="display: flex;justify-content: space-between;" v-if="state == 1 && currencyIdx == 1">
				...
				<view v-for="(item, index) in list" :key="index" class="plant">
					<view @click="checkDetails(item)">
						<view class="item">
							<view class="row">
								<view class="img">
									<image class="img" v-if="item.img" :src="imgUrl + item.img" mode="aspectFill">
									</image>
								</view>
								<view style="flex: 1">
									...
									<view v-if="state == 1">
										<view v-if="currencyIdx == 0">
											<view class="cs">售价：
												<text>{{ getPrice(item.saleCnyOrHkdMoney, 'CNY') }}</text>
											</view>
											<view class="cs">利润：
												<text
													:style="{color: item.profit < 0 ? 'red' : '#999'}">{{ getPrice(item.profit, 'CNY') }}</text>
											</view>
										</view>
...
<script>
	...
	import sofarPicker from '@/jewelryPackage/sofar-picker.vue'
	...
	export default {
		...
		mounted() {
			...
			query.select('#inventory-top').boundingClientRect(data => {
				console.log('顶部高度');
				console.log(data);
		...
		methods: {
			...
			search() {
				...
				uni.request({
					url: this.$baseJewelleryUrl + "/jewellerySalesReport?page=" + this.page + '&pageNum=10',
					method: "POST",
					header: {
			...
			exportData() {
				...
				uni.request({
					url: this.$baseJewelleryUrl + "/salesReportExport",
					method: "POST",
					header: {
						"content-type": "application/json",
						token: uni.getStorageSync("token"),
					...
					complete: (res) => {
						...
						if (res.statusCode === 200) {
							let url = this.$baseJewelleryUrl + res.data.url;
			...
			getGroupList() {
				uni.request({
					url: this.$baseJewelleryUrl + "/groupList",
					header: {
						"content-type": "application/json",
						token: uni.getStorageSync("token"),
					},
