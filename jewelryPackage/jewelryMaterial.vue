<template>
	<view class="content">
		<!-- 珠宝原材料 -->
		<popwndSelect :visible.sync="bShowStock" strTitle="请选择地点" :lst="stocks" strName="name" :sel="stockIdx"
			@confirm="stockChange">
		</popwndSelect>
		<popwndSelect :visible.sync="bShowMaterial" strTitle="请选择裸石类型" :lst="materialModelList" strName="name"
			:sel="materialIdx" @confirm="materialChange">
		</popwndSelect>
			...
			<view class="inputs">
				...
				<view class="material-filter">
					...
					<view class="filter-every" @click="filterSel(2)">
						<view class="every-main">
							<text class="every-font" :style="{ color: sortType == 2 ? '#000' : '#c3c3c3' }">单价</text>
							<view class="every-img">
								<image :src="
					      sortType == 2 && isUp == 1
					        ? img1
					...
					<view class="filter-every" @click="filterSel(3)">
						<view class="every-main">
							<text class="every-font" :style="{ color: sortType == 3 ? '#000' : '#c3c3c3' }">总价</text>
							<view class="every-img">
								<image :src="
					      sortType == 3 && isUp == 1
					        ? img1
					...
					<view class="filter-every" @click="filterSel(4)">
						<view class="every-main">
							<text class="every-font" :style="{ color: sortType == 4 ? '#000' : '#c3c3c3' }">重量</text>
							<view class="every-img">
								<image :src="
					      sortType == 4 && isUp == 1
					        ? img1
			...
			<view class="plantList">
				<view v-for="(item, index) in list" :key="index" class="plant">
					<view @click="checkDetails(item)">
						<view class="item">
							<view class="row">
								<image class="img" v-if="item.img" :src="imgUrl + item.img.split('|')[0]"
									mode="aspectFill">
								</image>
								<view>
									...
									<view style="display: flex; justify-content: space-between; align-items: center;">
										<view>
											...
											<view class="cs" v-if="item.quality">品质：{{ item.quality }}</view>
											<view class="cs" v-if="item.subsection">品质细分：{{ item.subsection }}</view>
