		...
		<div class="flex align-stretch" @click="onClick(item)" @longpress="long">
			<view class="flex align-center justify-center position-relative" style="width: 145rpx;">
				...
				<free-badge badgeClass="position-absolute" badgeStyle="top:15rpx;right:15rpx"
					v-if="item.num>0&&item.receiver==isself" :value="item.num"></free-badge>
			</view>
			<view class="flex flex-column border-bottom flex-1 py-3 pr-3 border-light-secondary">
				<view class="flex align-center justify-between mb-1">
					<text class="font-md">{{item.nick}}</text>
					<!-- <text class="font-sm text-light-muted">{{item.time|formatTime}}</text> -->
				</view>
				<text class="font text-ellipsis text-light-muted" v-if="item.contentType==0">{{item.contentText}}</text>
...
<script>
	...
	export default {
		...
		methods: { 
			onClick(emit) {
				let obj={
					head_pic:emit.headPic,
					username:emit.nick
				}
				uni.setStorageSync('chater',obj)
				console.log("客服",emit)
				console.log(uni.getStorageSync('chater'))
				uni.navigateTo({
					url: '/pages/message/chat/chat/chat?id=' + emit.id,
					fail: (res) => {
