<template>
	<view class="content">
		<popwndSelect :visible.sync="bShowCompany" strTitle="请选择公司" :lst="companys" strName="companyName" :sel="companyIdx" @confirm="companyChange">
		</popwndSelect>
		<popwndSelect :visible.sync="bShowStock" strTitle="请选择仓库" :lst="stocks" strName="warehouseName" :sel="stockIdx" @confirm="stockChange">
		</popwndSelect>
		...
		<view class="inventory-top">
			...
			<view class="mine-title">
				<image @click="goBack" style="width: 17rpx; height: 30rpx" src="../static/imgs/common/back.png" mode="aspectFill"></image>
				<input class="search-input" type="text" v-model="keyWord" placeholder="搜索" placeholder-style="color: #7b7b7b;font-size: 24rpx;" @input="inputChange" />
			</view>
			<!--  #endif -->
			<!--  #ifndef  MP-WEIXIN -->
				...
				<view v-for="(item, index) in list" :key="index" class="item" @click="checkDetails(item)">
					...
					<view>
						...
						<view style="display: flex; align-items: center;">
							...
							<view style="min-height: 360rpx; margin-left: 30rpx; display: flex; flex-direction: column; justify-content: space-between;">
								...
								<view style="margin-top: 40rpx;">
									...
									<view v-if="item.pricePeer" class="attri">同行价：{{ getPrice(item.pricePeer, currency, '-', 0) }}</view>
									<view v-if="item.priceIndi" class="attri">散客价：{{ getPrice(item.priceIndi, currency, '-', 0) }}</view>
