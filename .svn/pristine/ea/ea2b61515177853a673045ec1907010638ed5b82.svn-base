		...
		<view v-else>
			...
			<checkbox-group @change="checkChange" style="padding-top: 120rpx">
				<view class="commoditybox" v-for="(item, index) in favoriteslist" :key="index">
					...
					<view class="box-left" v-if="item.type == 2" @click="navto(item)">
						<image v-if="item.img" :src="watchUrl + item.img.trim()" mode="aspectFill">
						</image>
						<image v-else src="../static/imgs/common/nopic.jpg" mode="aspectFill"></image>
					</view>
                ...
                item.adviseSellPrice == 0
                  ? "价格请咨询客服"
                  : item.currency + " " + formatNumberRgx(item.adviseSellPrice)
              }}
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			favorites() {
				...
				uni.request({
					url: this.$baseUrl + "/wechat/api/browseRecords",
					header: {
			...
			clickclose() {
				uni.request({
					method: "DELETE",
					url: this.$baseUrl + "/wechat/api/browseRecordDel",
					data: {
